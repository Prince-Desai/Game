//
//  HardLeavelViewController.swift
//  Game
//
...
class HardLeavelViewController: UIViewController {
...
extension HardLeavelViewController{
    private func displayAlert(massage: String){
        let alert: UIAlertController = UIAlertController(title: "Game Over\n", message: "Score: \(point)\n High score: \(point)", preferredStyle: .alert)

        let okButten: UIAlertAction = UIAlertAction(title: "Play Again", style: .default)
    ...
    func updateTime(){
        if timerCount > 0{
            timerCount = timerCount - 0.04
            progressBar.setProgress(Float(timerCount/4.0), animated: true)
        }else{
            displayAlert(massage: "game Over")
            timer.invalidate()
        }
